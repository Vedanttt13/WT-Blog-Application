...
const blogsSlice = createSlice({
  ...
  reducers: {
    ...
    setBlogsSuccess: (state, { payload }) => {
      state.loading = false;
      const newBlogs = payload.data;
      const existingBlogs = state.blogsData.blogsList;
    
      // Create a map to store unique blogs by their ID
      const blogMap = new Map();
    
      // Add existing blogs to the map
      existingBlogs.forEach(blog => blogMap.set(blog._id, blog));
    
      // Add new blogs to the map (this will overwrite duplicates)
      newBlogs.forEach(blog => blogMap.set(blog._id, blog));
    
      // Convert the map back to an array
      state.blogsData.blogsList = Array.from(blogMap.values());
    ...
    setBookmarkedBlogSuccess: (state, { payload }) => {
      state.loading = false;
      const newBlogs = payload.data;
      const existingBlogs = state.bookmarkedBlogs.blogsList;
      ...
      // Convert the map back to an array
      state.bookmarkedBlogs.blogsList = Array.from(blogMap.values());
    ...
    setSearchedBlogsSuccess: (state, { payload }) => {
      state.loading = false;
      const newBlogs = payload.data;
      const existingBlogs = state.searchedBlogs.blogsList;
      ...
      // Convert the map back to an array
      state.searchedBlogs.blogsList = Array.from(blogMap.values());
    ...
    setUserPublishedBlogsSuccess: (state, { payload }) => {
      state.loading = false;
      const newBlogs = payload.data;
      const existingBlogs = state.userPublishedBlogs.blogsList;
      ...
      // Convert the map back to an array
      state.userPublishedBlogs.blogsList = Array.from(blogMap.values());
